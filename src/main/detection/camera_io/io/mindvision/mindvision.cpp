...
void MindVision::open()
{
  ...
  CameraSetClrTempMode(handle_, CT_MODE_USER_DEF);         // 手动白平衡
  CameraSetUserClrTempGain(handle_, 155, 98, 201);         // 硬编码值rgb

  // std::cout << "white balance return: " << CameraSetWbMode(handle_, true) << std::endl;
  // CameraSetOnceWB(handle_);
