...
namespace tools
{
  Recorder::Recorder(double fps,std::string folder_path) : init_(false), fps_(fps) , folder_path_(folder_path)
  {
    ...
    video_path_ = fmt::format("{}/{}.mp4", folder_path_, file_name);
  ...
  frame_queue_.push({img.clone(), timestamp});
  
  // 将图像帧和时间戳放入队列
  // while (tools::delta_time(timestamp, start_time_) / (1.0 / fps_) > frame_count_) {
    // std::lock_guard<std::mutex> lock(mtx_);
    // frame_queue_.push({img.clone(), timestamp});
    // frame_count_++;
  // }
  ...
  {
    auto fourcc = cv::VideoWriter::fourcc('H', '2', '6', '4');  //AVC1
  ...
  void Recorder::writeVideo()
  {
    while (!stop_flag_)
    {
      ...
      // while (frame_queue_.size() > BUFFER_SIZE) {
      //   frame_queue_.pop();
      // }
