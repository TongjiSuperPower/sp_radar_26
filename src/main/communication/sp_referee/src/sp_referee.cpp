...
namespace sp_referee
{
    ...
    bool Referee::init()
    {
        ...
        radar_ally_combined_sub_ = this->create_subscription<radar_msgs::msg::RadarAllyCombinedData>(
            "/radar_ally_combined_data", 10, std::bind(&Referee::radarAllyCombinedDataCallback, this, std::placeholders::_1));
        // 接收decision统一队列的发送指令，触发实际串口写入
        interactive_data_cmd_sub_ = this->create_subscription<radar_msgs::msg::InteractiveDataCmd>(
            "/interactive_data_cmd", 10, std::bind(&Referee::interactiveDataCmdCallback, this, std::placeholders::_1));
    ...
    {
        radar_cmd_ref_ = *msg;
        // 只缓存数据，实际发送由 interactiveDataCmdCallback 触发
    }
    ...
    void Referee::radarAllyCombinedDataCallback(const radar_msgs::msg::RadarAllyCombinedData::ConstPtr &msg)
    {
        radar_ally_combined_ref_ = *msg;
        // 只缓存数据，实际发送由 interactiveDataCmdCallback 触发
    }

    void Referee::interactiveDataCmdCallback(const radar_msgs::msg::InteractiveDataCmd::ConstPtr &msg)
    {
        write(sp_referee::ROBOT_INTERACTIVE_DATA_CMD, msg->child_cmd, msg->receiver_id);
    }
