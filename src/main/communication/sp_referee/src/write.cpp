...
{
    void Referee::write(uint16_t write_cmd, uint16_t child_cmd, uint16_t receiver_id)
    {
        RCLCPP_INFO(rclcpp::get_logger("Referee"), "write() called: write_cmd=0x%04X (%d), child_cmd=0x%04X (%d), receiver_id=%d",
                    write_cmd, write_cmd, child_cmd, child_cmd, receiver_id);
        ...
        case sp_referee::ROBOT_INTERACTIVE_DATA_CMD:
        {
            if (robot_info_.robot_type_ == "radar")
            {
                ...
                case sp_referee::RADAR_CMD_CMD:
                {
                    ...
                    radar_cmd.robot_interaction_data_header_.receiver_id_ = receiver_id;
                ...
                case sp_referee::RADAR_SENTRY_POSITION_CMD:
                {
                    ...
                    radar_sentry_position_cmd.robot_interaction_data_header_.receiver_id_ = receiver_id;
